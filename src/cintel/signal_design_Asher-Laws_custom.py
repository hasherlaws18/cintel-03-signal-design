...
Terminal command to run this file from the root project folder
    ...
    uv run python -m cintel.signal_design_Asher-Laws_custom
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_Asher-Laws.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_Asher-Laws_custom.csv"
...
def main() -> None:
    ...
    error_rate_signal_recipe: pl.Expr = (
        ...
        .then(calculated_error_rate.round(4))
        .otherwise(0.0)
        .alias("error_rate")
    )
    ...
    avg_latency_signal_recipe: pl.Expr = (
        pl.when(is_requests_positive)
        .then(calculated_avg_latency.round(2))
        .otherwise(0.0)
